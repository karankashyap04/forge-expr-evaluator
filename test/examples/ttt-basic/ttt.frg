...
-- dummy predicates specifically for testing expr evaluator
pred argPred1[x: Int, y: Int] {
    x = y
    y = 3
}

pred argPred2[x: Int, y: Int] {
    x = y
    argPred1[2, add[x, 1]]
}
...
-- end of dummy predicates

run {
    all b: Board | wellformed[b]
    traces
} for exactly 7 Board, 4 Int for {next is linear}
